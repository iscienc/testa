...
void ProcessEntrySignals()
  {
   ...
   if(!CheckAllFilters())
     {
      Print("[SMDBG] blocked: CheckAllFilters");
      return;
     }
...
// Step 2: Try counter-direction chains (presets 7, 8, CUSTOM counter-dir)
   ...
   if(!tradeFound)
     {
      PrintFormat("[SMDBG] blocked: no tradable chain, reason=%s, active=%d", narReason, SM_CountActiveInstances());
      return;
     }
   ...
   if(!CheckForEntryTrigger(isBullish))
     {
      Print("[SMDBG] blocked: CheckForEntryTrigger");
      return;
     }
   if(!ValidateEntryConditions(isBullish))
     {
      Print("[SMDBG] blocked: ValidateEntryConditions");
      return;
     }
   if(!CheckEntryConfirmations(isBullish))
     {
      Print("[SMDBG] blocked: CheckEntryConfirmations");
      return;
     }
