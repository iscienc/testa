...
//================================================================
   ...
   g_posTracking.Reset();
   g_posTracking.ticket         = (result.deal > 0) ? result.deal : result.order;
...
// tracked pending order's chain, preserve the structural TP set.
   if(g_posTracking.ticket != ticket)
     {
      if(g_posTracking.pendingOrderTicket > 0 &&
         g_posTracking.chainId > 0 &&
         StringFind(PositionGetString(POSITION_COMMENT),
                    "#" + IntegerToString(g_posTracking.chainId)) >= 0)
        {
         ...
         g_posTracking.pendingOrderTicket = 0;
         Print("[TRADE] Pending order filled; structural TP levels preserved for chain #",
               g_posTracking.chainId);
        }
      else
        {
...
bool ModifyPositionSL(ulong ticket, double newSL)
  {
   ...
   if(!ValidateBrokerStops(closeType, reference, request.sl, request.tp))
     {
      ...
      if(g_mlInitialized && InpML_Mode != ML_OFF)
        {
         ...
         if(g_posTracking.hasSignalFeatures)
           ...
           {
            // Recovery path only: the EA was restarted or the signal
            // was created before feature snapshot support was installed.
            // Normal trades must always use hasSignalFeatures=true.
            Print("[ML] Signal-time feature snapshot unavailable; using restart-recovery fallback");
            fv = ExtractFeatures();
           }

         long dealType = HistoryDealGetInteger(dealTicket, DEAL_TYPE);
         ENUM_SIGNAL_TYPE sigType =
            (dealType == DEAL_TYPE_BUY) ? SIGNAL_BUY : SIGNAL_SELL;

         if(!g_posTracking.hasSignalFeatures)
            Print("[ML] WARNING: current closing trade has no signal-time feature snapshot");
