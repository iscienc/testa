//+------------------------------------------------------------------+
//|                        ICT_Unified_EA_v20.mq5                    |
//|              Professional Unified ICT Trading System             |
//|                         Version 20                               |
//+------------------------------------------------------------------+
#property copyright "ICT Unified Professional EA v20"
#property link      ""
...
int OnInit()
  {
   Print("===========================================================");
   Print("ICT Unified Professional EA v20 - Initializing...");
   Print("===========================================================");
...
// Keep this: SM detectors rely on OB/FVG/OTE data from this layer.
 ...
 if(!InitializeKillzones())
   ...
   //--- FIX C1 (guard) -----------------------------------------------
   // Prove session state is LIVE before allowing the killzone filter to
   // gate trading. After a successful UpdateKillzoneStatus() the state
   // is never KZ_NONE (it falls through to KZ_OFF_HOURS), so this is a
   // genuine assertion, not a tautology.
   UpdateKillzoneStatus();
   if(InpUseKillzoneFilter && g_killzone.current == KZ_NONE)
     {
      Print("===========================================================");
      Print("FATAL: InpUseKillzoneFilter is ON but session state failed");
      Print("       to initialise. Every chain would be rejected at");
      Print("       NAR_EnvironmentOK with 'environment_filter'.");
      Print("===========================================================");
      return INIT_FAILED;
     }
   Print("Killzone runtime live. Current session: ", GetKillzoneDescription());
   ...
   Print("===========================================================");
   Print("ICT Unified EA V20 initialized successfully");
...
void OnDeinit(const int reason)
  {
   ...
   Print("ICT Unified EA V20 deinitialized. Reason: ", reason);
  }
...
//--- FIX C1 ---------------------------------------------------
      // Session / killzone state is a CROSS-CUTTING ENVIRONMENTAL
      // service, not a pattern detector. It must never be demand-gated
      // by the SM element registry, and it must be resolved BEFORE any
      // consumer runs:
      //   DetectJudasSwings()   -> reads g_killzone.isActive
      //   NAR_EnvironmentOK()   -> reads IsInKillzone()
      //   CheckSessionFilter()  -> reads g_killzone.current
      // Cost is ~2 microseconds. g_needDetectKillzone is demoted to a
      // DRAW / TELEMETRY flag only (see g_perf.loadedFamilies below).
      UpdateKillzoneStatus();
