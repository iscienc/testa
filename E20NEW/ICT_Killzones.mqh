...
//--- FIX C1 / M9 --------------------------------------------------
// On TRANSITION: hard-reset the session window so the incoming
// session starts from a clean high/low.
// On EVERY call: extend the window. The original only ran on
// transition, so sessionHigh/sessionLow were captured once at the
// session boundary and then frozen for the whole session, making
// GetSessionRange() unusable.
   if(g_killzone.current != previousKZ)
     {
      g_killzone.sessionHigh = 0.0;
      g_killzone.sessionLow  = 0.0;
      g_killzone.openPrice   = 0.0;
     }
