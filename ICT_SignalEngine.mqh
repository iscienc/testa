...
//+------------------------------------------------------------------+
//| FIX C7                                                           |
//| A take-profit is a DISTANCE from entry scaled by an R multiple.   |
//| Never derive a price from a price. The previous implementation    |
//| computed tp2 = tp1 * 1.5, which on EURUSD turned TP1 = 1.0850     |
//| into TP2 = 1.6275 -> either broker rejection (10016 invalid       |
//| stops) or an accepted position with effectively no take-profit.   |
//|                                                                   |
//| This function is a FALLBACK VALIDATOR, not a TP shaper. It only   |
//| repairs levels that ended up on the wrong side of entry. It       |
//| deliberately does NOT reorder or spread valid levels, so          |
//| TP_FIXED_RR (which intentionally collapses tp1 == tp2 == tp3)     |
//| keeps its semantics.                                              |
//+------------------------------------------------------------------+
void ValidateTPDirection(bool isBullish, double entry)
  {
   ...
   // Degrade gracefully: SL -> ATR -> a 10-point floor. risk must never
   // be zero, or every repaired TP collapses onto entry and
   // ValidateSignal() silently kills the signal.
   if(risk <= 0.0)
      risk = GetATR();
   if(risk <= 0.0)
      risk = 10.0 * SymbolInfoDouble(_Symbol, SYMBOL_POINT);

   double rr1 = MathMax(InpTP1_RR, 0.1);
   double rr2 = MathMax(InpTP2_RR, rr1);
   double rr3 = MathMax(InpTP3_RR, rr2);

   if(isBullish)
     {
      if(g_currentSignal.tp1Price <= entry)
         g_currentSignal.tp1Price = entry + risk * rr1;
      if(g_currentSignal.tp2Price <= entry)
         g_currentSignal.tp2Price = entry + risk * rr2;
      if(g_currentSignal.tp3Price <= entry)
         g_currentSignal.tp3Price = entry + risk * rr3;
     }
   else
     {
      if(g_currentSignal.tp1Price >= entry)
   ...
   // CalculateTakeProfits() normalises BEFORE calling us, so anything
   // repaired here would otherwise be submitted un-normalised.
   g_currentSignal.tp1Price = NormalizePrice(g_currentSignal.tp1Price);
   g_currentSignal.tp2Price = NormalizePrice(g_currentSignal.tp2Price);
   g_currentSignal.tp3Price = NormalizePrice(g_currentSignal.tp3Price);
  }
