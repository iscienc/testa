...
// 1. DISTRIBUTION: If we have a BOS, we are in Distribution (trend is active)
  // FIX H2: phase precedence using only helpers that exist in this file.
// BOS alone is not Distribution. Require displacement + BOS.
if(hasRecentBOS && hasRecentDisplacement)
  ...
  {
   // No sweep and no confirmed BOS means the range is still forming.
   // Keep the existing accumulation-range test, but do not use the
   // absence of accWasSeen as proof by itself.
   if(currentBarRange < atr * InpAccumulationRangeATR)
     {
     ...
     {
      detectedPhase = g_amdPhase.currentPhase;
      confidence = MathMax(0, g_amdPhase.confidence - 5);
     }
  }
else
  {
   // Sweep + BOS is transitional. Do not label it Distribution until
   // the displacement-qualified confirmation is present.
   detectedPhase = AMD_MANIPULATION;
   confidence = 65;
  }
...
// FIX H2: compare against the phase captured BEFORE assignment.
// At this point g_amdPhase.currentPhase already equals detectedPhase,
// so the old condition was impossible.
   if(previousPhase != AMD_DISTRIBUTION &&
      detectedPhase == AMD_DISTRIBUTION &&
      ...
      g_amdPhase.distributionHigh > 0.0)
     {
      ...
      Print("✓ AMD CYCLE #", g_amdPhase.amdCycle,
            " COMPLETED | Range Size: ", rangeSize, " points");
     }
