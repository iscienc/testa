...
void DetectBullishOB(int barIndex, double atr)
  {
   ...
   if(InpOB_DisplacementCandles <= 1)
     {
      ...
      double nextCloseLocation =
         (nextRange > 0.0)
         ? (nextClose - nextLow) / nextRange
         : 0.0;

      bool bullishDisplacement =
         nextClose > nextOpen &&
         nextRange >= atr * InpOB_MinDisplacementATR &&
         nextBody >= nextRange * (InpDisp_MinBodyPercent / 100.0) &&
         nextCloseLocation >= 0.70 &&
         nextClose > obHigh;
   ...
   SSMStructuralEvent ctfEvent;
   if(SM_GetRecentStructuralEvent(LAYER_CTF, 3, ctfEvent))
     {
      int evBar = iBarShift(_Symbol, PERIOD_CURRENT, ctfEvent.time, false);
      if(evBar >= 0 && MathAbs(evBar - barIndex) <= 3)
        {
         g_orderBlocks[idx].causalTag = ctfEvent.tag;
      ...
      if(priceInOB && g_orderBlocks[i].status == OB_FRESH)
        {
         ...
         if(priceInOB && g_orderBlocks[i].status == OB_TESTED)
           {
      ...
      if(g_orderBlocks[i].type == OB_BULLISH)
        {
         ...
         double invalidationClose =
            iClose(_Symbol, PERIOD_CURRENT, 1);

         if(invalidationClose < g_orderBlocks[i].bottom)
           {
            for(int j = 1; j <= 3; j++)
              {
               double barRange = CandleRange(PERIOD_CURRENT, j);
               double barBody = BodySize(PERIOD_CURRENT, j);
               double barOpen = iOpen(_Symbol, PERIOD_CURRENT, j);
               double barClose = iClose(_Symbol, PERIOD_CURRENT, j);

               double closeLocation =
                  (barRange > 0.0)
                  ? (barClose - iLow(_Symbol, PERIOD_CURRENT, j))
                  / barRange
                  : 1.0;

               if(barRange >= atr * InpBreaker_MinDisplacementATR &&
                  barBody >= barRange * (InpDisp_MinBodyPercent / 100.0) &&
         ...
         if(g_orderBlocks[i].type == OB_BEARISH)
           {
            ...
            if(invalidationClose > g_orderBlocks[i].top)
              {
               for(int j = 1; j <= 3; j++)
                 {
                  double barRange = CandleRange(PERIOD_CURRENT, j);
                  ...
                  double closeLocation =
                     (barRange > 0.0)
                     ? (iHigh(_Symbol, PERIOD_CURRENT, j) - barClose)
                     / barRange
                     : 1.0;
...
void ConvertOBToBreaker(int obIndex, ENUM_BREAKER_TYPE breakerType)
  {
   ...
   for(int b = 0; b < g_breakerCount; b++)
     {
      if(g_breakerBlocks[b].time == g_orderBlocks[obIndex].time &&
         g_breakerBlocks[b].type == breakerType)
        {
         return;
        }
     }
...
void DetectRejectionBlocks()
  {
   ...
   if(atr <= 0)
      return;
   datetime cutoff = iTime(_Symbol, PERIOD_CURRENT, 0)
                     - (datetime)(InpOB_MaxAge_Hours * 3600);

   for(int r = g_rejectionCount - 1; r >= 0; r--)
     {
      if(g_rejectionBlocks[r].time >= cutoff)
         continue;

      DeleteObject(g_rejectionBlocks[r].objName);
      for(int j = r; j < g_rejectionCount - 1; j++)
         g_rejectionBlocks[j] = g_rejectionBlocks[j + 1];
      g_rejectionCount--;
     }
      ...
      if(lowerWick > bodySize * 2.0 && lowerWick > upperWick * 1.5)
        {
         ...
         if(g_rejectionCount >= g_maxRejections)
           {
            int oldest = 0;
            for(int j = 1; j < g_rejectionCount; j++)
               if(g_rejectionBlocks[j].time < g_rejectionBlocks[oldest].time)
                  oldest = j;

            DeleteObject(g_rejectionBlocks[oldest].objName);
            for(int j = oldest; j < g_rejectionCount - 1; j++)
               g_rejectionBlocks[j] = g_rejectionBlocks[j + 1];
            g_rejectionCount--;
           }
...
//|                                                                  |
//+------------------------------------------------------------------+
bool IsPriceAtRejectionBlock(bool lookForBullish, int &outIndex)
  {
   outIndex = -1;
   double price = iClose(_Symbol, PERIOD_CURRENT, 1);

   for(int i = 0; i < g_rejectionCount; i++)
     {
      SRejectionBlock block = g_rejectionBlocks[i];
      if(block.direction != (lookForBullish ? DIR_BULLISH : DIR_BEARISH))
         continue;

      double top = MathMax(block.wickTop, block.bodyTop);
      double bottom = MathMin(block.wickBottom, block.bodyBottom);
