...
// ★ FIX 3: if the entry stage has no element, there is no zone to require.
   ...
   if(SM_Detect_FVG(eLay, entryDir, false, tag, px))
      return true;
      if(SM_Detect_IFVG(eLay, entryDir, false, tag, px))
   return true;
if(SM_Detect_FVG_CE(eLay, entryDir, false, tag, px))
   return true;
if(SM_Detect_VolumeImbalance(eLay, entryDir, false, tag, px))
   return true;
if(SM_Detect_LiquidityVoid(eLay, entryDir, false, tag, px))
   return true;
