...
void SM_BuildLoadedElementSet()
  {
   ...
   g_needDetectOB =
      ...
      SM_IsElementLoaded(SM_ELEM_REJECTION_BLOCK);
...
// ★ Capture both slot prices for later zone-based entry
   ...
   if(cfg.primaryElem != SM_ELEM_NONE &&
   inst.stagePrimaryPrice[s] <= 0.0 &&
   (cfg.primaryElem == SM_ELEM_ORDER_BLOCK ||
    cfg.primaryElem == SM_ELEM_FVG ||
    cfg.primaryElem == SM_ELEM_IFVG ||
    cfg.primaryElem == SM_ELEM_FVG_CE ||
    cfg.primaryElem == SM_ELEM_VOLUME_IMBALANCE ||
    cfg.primaryElem == SM_ELEM_LIQUIDITY_VOID ||
    cfg.primaryElem == SM_ELEM_BREAKER ||
    cfg.primaryElem == SM_ELEM_MITIGATION ||
    cfg.primaryElem == SM_ELEM_OTE_ZONE ||
    cfg.primaryElem == SM_ELEM_REJECTION_BLOCK))
  {
   PrintFormat("[SM][CONTRACT] stage=%d element=%s satisfied without zone price",
               s, SM_ElementShortName(cfg.primaryElem));
  }
...
// Seed dummy direction so FROM_DR / FROM_AMD policies can resolve
   ...
   bool primOK = SM_CheckElementSatisfied(tcfg.primaryElem,
                                          tcfg.primaryTF,
                                          dummy,
                                          tcfg,
                                          ptag,
   ...
   if(tcfg.logic != SM_LOGIC_SINGLE && tcfg.secondaryElem != SM_ELEM_NONE)
     {
      bool secOK = SM_CheckElementSatisfied(tcfg.secondaryElem,
                                            tcfg.secondaryTF,
                                            dummy,
                                            tcfg,
                                            stag,
...
//--- FIX C4: trigger identity comes from the trigger's own TF lane.
// An unrelated later LTF event must not erase an HTF ChoCh trigger.
   SSMStructuralEvent triggerEvent;
   bool hasTriggerEvent = SM_GetLatestStructuralEvent(tcfg.primaryTF, triggerEvent);
   int currentEventTag = hasTriggerEvent ? triggerEvent.tag : -1;
   ...
   datetime triggerTime = hasTriggerEvent
                          ? triggerEvent.time
...
//--- FIX C6: stage zero must satisfy the same data contract as every
//    other stage. Without these two fields, TRIG_PRIMARY and
//    TRIG_SECONDARY entry zones are always zero and the signal is
//    silently discarded by SM_ResolveEntryZonePrice().
   ni.stageDone[0]           = true;
   ni.stageTime[0]           = TimeCurrent();
   ni.stageBarCtr[0]         = g_smBarCounter;
   ni.stagePrimaryPrice[0]   = pprice;
   ni.stageSecondaryPrice[0] = sprice;
   ni.currentStage           = 1;
...
// If the configured trigger element has no zone price (for example a
// ChoCh, BOS or displacement event), the trigger price is still useful
// for diagnostics, but the selected zone must fail explicitly rather
// than accidentally using zero. Zone-based entry requires a positive
// detector price by design.
   if(tcfg.primaryElem != SM_ELEM_NONE && pprice <= 0.0 &&
      (InpSM_EntryZone == SM_ZONE_TRIG_PRIMARY))
     {
      PrintFormat("[SM][EntryZone] Trigger primary '%s' produced no zone price; choose a zone element or SM_ZONE_CURRENT.",
                  SM_ElementShortName(tcfg.primaryElem));
     }
...
// Empty-slot guard (your requested warning)
   ...
   if(px <= 0.0)
     {
      reason = "zone_price_unset";
      PrintFormat("[SM][EntryZone] '%s' is configured, but stage %d '%s' returned no positive zone price.",
                  SM_ZoneName(z), stageIdx, SM_ElementShortName(elem));
      return false;
     }
...
// FIX 6E: one immutable feature snapshot belongs to this signal.
// Capture it before the signal is published or the instance is consumed.
   SMLFeatureVector signalFeatures;
   signalFeatures.Reset();
   bool hasSignalFeatures = false;

   if(g_mlInitialized && InpML_Mode != ML_OFF)
     {
      signalFeatures = ExtractFeatures();
      hasSignalFeatures = true;
     }
...
// Store the snapshot on the signal object. This happens after the
// pending branch recalculates entry/SL/TP, so the feature snapshot is
// still signal-time data but the signal is now fully formed.
   g_currentSignal.signalFeatures = signalFeatures;
   g_currentSignal.hasSignalFeatures = hasSignalFeatures;
...
// Find best active instance
   ...
   const double presetMax = (double)SM_PRESET_BULLISH_SWEEP_BEARISH_ENTRY;
   double presetNorm = (presetMax > 0.0)
                       ? (double)InpSM_Preset / presetMax
                       : 0.0;
   fv.Set(MLF_SM_PRESET_ID, MathMin(1.0, MathMax(0.0, presetNorm)));
