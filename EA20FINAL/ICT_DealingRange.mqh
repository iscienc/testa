...
bool InitializeStructureLayer()
  {
   g_bullDR.Reset();
   g_bearDR.Reset();
   g_bullDR.timeframe = PERIOD_CURRENT;
   g_bullDR.tfLayer = LAYER_CTF;
   g_bearDR.timeframe = PERIOD_CURRENT;
   g_bearDR.tfLayer = LAYER_CTF;
...
void ScanInitialStructure()
  {
   ...
   if(scanBars < 30)
     {
      Print("Not enough bars for initial scan");
      return;
     }
...
void CheckCorrectionLineUpdate(SDealingRange &dr, bool isBullish)
  {
   ...
   if(pivotBar < InpCL_PivotRightBars)
      return;

   bool isPivot;
   if(isBullish)
      isPivot = IsPivotHigh(PERIOD_CURRENT,pivotBar,InpCL_PivotLeftBars,InpCL_PivotRightBars);
   else
      isPivot = IsPivotLow(PERIOD_CURRENT,pivotBar,InpCL_PivotLeftBars,InpCL_PivotRightBars);

   if(isPivot)
     {  UpdateCorrectionLine(dr,isBullish,dr.corrLine.pendingExtreme,dr.corrLine.pendingExtremeTime); }
   else
      if(pivotBar > InpCL_ForceUpdateBars)
        {  UpdateCorrectionLine(dr,isBullish,dr.corrLine.pendingExtreme,dr.corrLine.pendingExtremeTime); }
      else
         if(InpCL_UpdateMode == CL_IMMEDIATE_EXTREME)
           {  UpdateCorrectionLine(dr,isBullish,dr.corrLine.pendingExtreme,dr.corrLine.pendingExtremeTime); }
  }
...
void CreateFirstOrigin(SDealingRange &dr, bool isBullish,
                       double originPrice, datetime originTime)
  {
   ...
   if(InpShowDR_Origins)
      DrawDR_Origin(idx, dr, isBullish);
...
bool RemoveOldestOrigin(SDealingRange &dr)
  {
   ...
   for(int i=oldIdx;i<dr.originCount-1;i++)
   ...
   for(int i = rB + 1; i < maxBars && cc < 50; i++)
     {
      ...
      if(isBullish)
        {
         isP = IsPivotLow(PERIOD_CURRENT, i, lB, rB);
         if(isP)
            pr = iLow(_Symbol, PERIOD_CURRENT, i);
        }
      else
        {
         isP = IsPivotHigh(PERIOD_CURRENT, i, lB, rB);
         if(isP)
            pr = iHigh(_Symbol, PERIOD_CURRENT, i);
        }
      if(!isP)
         continue;
      ...
      if(!isBullish && pr <= dr.corrLine.extremePrice)
         continue;
      if(IsLevelBroken(PERIOD_CURRENT, pr, isBullish, i))
         continue;
      ...
      if(ps < InpExtMinPivotScore)
         continue;
      cP[cc]=pr;
      cT[cc]=bT;
      cB[cc]=i;
      cS[cc]=depth+(ps*atr*0.1);
      cc++;
     }
...
void AddExternalInducement(SDealingRange &dr, double price, datetime time,
                           int barIndex, bool isBullish)
  {
   ...
   if(dr.externalCount>=InpMaxExtInducements)
      RemoveOldestExternal(dr,isBullish);
   ...
   if(InpShowDR_Externals)
      DrawDR_External(idx,dr,isBullish);
  }
...
void RemoveOldestExternal(SDealingRange &dr, bool isBullish)
  {
   if(dr.externalCount==0)
      return;
   int oI=0;
   datetime oT=dr.externals[0].time;
   for(int i=1;i<dr.externalCount;i++)
      if(dr.externals[i].time<oT)
        {
         oT=dr.externals[i].time;
         oI=i;
        }
   DeleteObject(dr.externals[oI].lineObjName);
   DeleteObject(dr.externals[oI].labelObjName);
   for(int i=oI;i<dr.externalCount-1;i++)
      dr.externals[i]=dr.externals[i+1];
   dr.externalCount--;
  }
...
void RemoveSweptExternal(SDealingRange &dr, int index)
  {
   ...
   for(int i=index;i<dr.externalCount-1;i++)
      dr.externals[i]=dr.externals[i+1];
   dr.externalCount--;
  }
...
void AddInternalLevel(SDealingRange &dr, double price, datetime time, bool isBullish)
  {
   ...
   dr.internalCount++;
   if(InpShowDR_Internals)
      DrawDR_Internal(idx,dr,isBullish);
  }
...
void ReclassifyInternalsToExternals(SDealingRange &dr, bool isBullish, datetime newCLTime)
  {
   ...
   for(int i=dr.internalCount-1;i>=0;i--)
     {
      if(dr.internals[i].time>=newCLTime)
         continue;
      DeleteObject(dr.internals[i].lineObjName);
      DeleteObject(dr.internals[i].labelObjName);
      if(!dr.internals[i].isBroken)
        {
         ...
         for(int k=0;k<dr.externalCount;k++)
            if(MathAbs(dr.externals[k].price-dr.internals[i].price)<minD)
              {
               tooClose=true;
               break;
              }
      ...
      for(int j=i;j<dr.internalCount-1;j++)
         dr.internals[j]=dr.internals[j+1];
      dr.internalCount--;
     }
  }
      ...
      if(dr.pullback.confirmed)
        {
         ...
         if(InpShowPullbackOrigin)
            DrawPullbackOrigin(dr, isBullish);
...
// C4: this event is a correction-line break, not a ChoCh.
...
void CheckDR_ChoCh(SDealingRange &dr, bool isBullish)
  {
   ...
   datetime pT=g_prevBarTime;
   if(pT<=dr.origins[chochIdx].time)
      return;

   bool originBroken=false;
   if(isBullish)
      originBroken=CheckBreak(PERIOD_CURRENT,dr.origins[chochIdx].price,false);
   else
      originBroken=CheckBreak(PERIOD_CURRENT,dr.origins[chochIdx].price,true);
   if(!originBroken)
      return;
...
// C4: this is the DR Origin / Dealing-Range ChoCh event.
   SM_RegisterStructuralEvent(brokenPrice,
                              chDir,
                              1,
                              LAYER_CTF,
                              SM_EVENT_CHOCH);
...
// Remove broken origin
   ...
   for(int i=chochIdx;i<dr.originCount-1;i++)
      dr.origins[i]=dr.origins[i+1];
   dr.originCount--;
...
// FIX C5: only the newly promoted lane is dominant. The opposite
// lane is historical unless it has its own active correction line.
// This assignment belongs AFTER both direction branches, so it is
// applied consistently for bullish->bearish and bearish->bullish.
   g_bullDR.isDominant = g_isBullishActive;
   g_bearDR.isDominant = !g_isBullishActive;
...
void UpdateSingleDR(SDealingRange &dr, bool isBullish)
  {
   if(!dr.corrLine.isActive && !dr.corrLine.needsUpdate)
      return;
   double atr=GetATR();
   if(atr<=0)
      return;
...
// Step 5: Track external extreme
   if(dr.externalSwept)
      TrackExtremeReached(dr, isBullish);
...
// FIX C5: update both direction lanes. The dominant DR still owns
// the narrative bias, but the opposite DR remains current so
// counter-direction and inverted-entry stages have real structure
// to inspect. Never change g_isBullishActive in this loop.
   if(g_bullDR.corrLine.isActive || g_bullDR.corrLine.needsUpdate)
      UpdateSingleDR(g_bullDR, true);

   if(g_bearDR.corrLine.isActive || g_bearDR.corrLine.needsUpdate)
      UpdateSingleDR(g_bearDR, false);
   ...
   if(g_isBullishActive)
     {
      ...
      for(int i=dr.externalCount-1;i>=0;i--)
         if(dr.externals[i].status==DR_ACTIVE)
           {
            lb=dr.externals[i].price;
            break;
           }
      int ci=GetChochOriginIndex(dr);
      if(ci>=0 && (lb==0||dr.origins[ci].price>lb))
         lb=dr.origins[ci].price;
      if(lb>0)
        {
         g_entryZone.lowerBound=lb;
         g_entryZone.isValid=true;
        }
     }
   else
     {
      ...
      for(int i=dr.externalCount-1;i>=0;i--)
         if(dr.externals[i].status==DR_ACTIVE)
           {
            ub=dr.externals[i].price;
            break;
           }
      int ci=GetChochOriginIndex(dr);
      if(ci>=0 && (ub==0||dr.origins[ci].price<ub))
         ub=dr.origins[ci].price;
      if(ub>0)
        {
         g_entryZone.upperBound=ub;
         g_entryZone.isValid=true;
        }
     }
...
void ExtendDR_Lines(SDealingRange &dr)
  {
   ...
   for(int i=0;i<dr.internalCount;i++)
      if(!dr.internals[i].isBroken)
         ExtendLineToCurrent(dr.internals[i].lineObjName);
...
void CheckTargetsReached()
  {
   ...
   for(int i=0;i<g_bullDR.originCount;i++)
     {
      if(g_bullDR.origins[i].role!=ROLE_TARGET||g_bullDR.origins[i].isReached)
         continue;
      if(pL<=g_bullDR.origins[i].price)
        {
         g_bullDR.origins[i].isReached=true;
         g_bullDR.origins[i].reachedTime=pT;
   ...
   for(int i=0;i<g_bearDR.originCount;i++)
     {
      if(g_bearDR.origins[i].role!=ROLE_TARGET||g_bearDR.origins[i].isReached)
         continue;
      if(pH>=g_bearDR.origins[i].price)
        {
         g_bearDR.origins[i].isReached=true;
         g_bearDR.origins[i].reachedTime=pT;
...
void ReclassifyExternalsToTargetLines(SDealingRange &oldDR, bool wasBullish)
  {
   ...
   for(int i=0;i<oldDR.externalCount;i++)
     {
      if(oldDR.externals[i].status!=DR_ACTIVE)
         continue;
      bool ok=false;
      if(newDir==DIR_BEARISH && oldDR.externals[i].price<cp)
         ok=true;
      if(newDir==DIR_BULLISH && oldDR.externals[i].price>cp)
         ok=true;
      if(ok)
         AddDRTargetLine(oldDR.externals[i].price,oldDR.externals[i].time,
   ...
   for(int i=0;i<oldDR.originCount;i++)
     {
      if(oldDR.origins[i].role==ROLE_TARGET && !oldDR.origins[i].isReached)
        {
         bool ok=false;
         if(newDir==DIR_BEARISH && oldDR.origins[i].price<cp)
            ok=true;
         if(newDir==DIR_BULLISH && oldDR.origins[i].price>cp)
            ok=true;
         if(ok)
            AddDRTargetLine(oldDR.origins[i].price,oldDR.origins[i].time,
...
void AddDRTargetLine(double price, datetime time, ENUM_TARGET_SOURCE source,
                     ENUM_TRADE_DIRECTION forDir)
  {
   for(int i=0;i<g_drTargetLineCount;i++)
      if(MathAbs(g_drTargetLines[i].price-price)<_Point*5)
         return;
   if(g_drTargetLineCount>=g_maxDRTargetLines)
      return;
   int idx=g_drTargetLineCount;
   g_drTargetLines[idx].Reset();
   g_drTargetLines[idx].price=price;
   g_drTargetLines[idx].time=time;
   g_drTargetLines[idx].source=source;
   g_drTargetLines[idx].forDirection=forDir;
   ...
   g_drTargetLineCount++;
   if(InpShowDR_TargetLines)
      DrawDR_TargetLine(idx);
  }
   ...
   for(int i=0;i<g_drTargetLineCount && assigned<3;i++)
     {
      if(g_drTargetLines[i].isReached)
         continue;
      if(g_drTargetLines[i].forDirection!=td)
         continue;
      if(isBullish && g_drTargetLines[i].price<=entryPrice)
         continue;
      if(!isBullish && g_drTargetLines[i].price>=entryPrice)
         continue;
      assigned++;
      if(assigned==1)
         tp1=g_drTargetLines[i].price;
      else
         if(assigned==2)
            tp2=g_drTargetLines[i].price;
         else
            tp3=g_drTargetLines[i].price;
     }
   return (assigned>=1);
  }
   ...
   for(int i=0;i<g_drTargetLineCount;i++)
     {
      ...
      if(g_drTargetLines[i].forDirection==DIR_BEARISH)
         ...
         if(g_drTargetLines[i].forDirection==DIR_BULLISH)
...
int CountUnreachedDRTargets(ENUM_TRADE_DIRECTION dir)
  {
   int c=0;
   for(int i=0;i<g_drTargetLineCount;i++)
      if(!g_drTargetLines[i].isReached && g_drTargetLines[i].forDirection==dir)
         c++;
   return c;
  }
