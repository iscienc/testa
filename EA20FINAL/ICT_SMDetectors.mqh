...
//=====================================================================
bool SM_Detect_ChoChBreak(ENUM_TF_LAYER tfLay,
                          ...
                          double &outPrice)
  {
   ...
   SSMStructuralEvent ev;
   if(!SM_GetRecentStructuralEvent(tfLay, 5, ev))
      return false;

   if(ev.eventType != SM_EVENT_CHOCH)
      return false;

   outDir = ev.direction;
   outPrice = ev.price;
   return (outDir != DIR_NONE && outPrice > 0.0);
  }
...
bool SM_Detect_BOS(ENUM_TF_LAYER tfLay, ENUM_TRADE_DIRECTION dir)
  {
   ...
   if(dr == NULL || !dr.isActive || !dr.corrLine.isActive)
      return false;
   if(dr.corrLine.extremeTime <= 0)
      return false;

   int bosBar = iBarShift(
                   _Symbol,
                   SM_LayerToTF(tfLay),
                   dr.corrLine.extremeTime,
                   false
                );
   ...
   if(causal)  // C3 gate
     {
      if(g_smActiveCausalTag < 0)
         return false;
      SSMStructuralEvent layerEvent;
      if(!SM_GetRecentStructuralEvent(tfLay, 20, layerEvent))
         return false;
     }
...
// CTF uses the stored FVG registry and its lifecycle state.
   if(tfLay == LAYER_CTF)
     {
      int idx = -1;
      int requiredTag =
         (causal && g_smActiveCausalTag >= 0)
         ? g_smActiveCausalTag
         : -1;

      if(IsPriceInIFVG(bull, requiredTag, idx))
        {
         if(idx < 0 || idx >= g_ifvgCount)
            return false;

         outTag = g_ifvgList[idx].causalTag;
         outPrice =
            (g_ifvgList[idx].top + g_ifvgList[idx].bottom) * 0.5;
   ...
   if(causal)
     {
      ...
      int eventAge =
         g_smBarCounter - g_lastSMEvent.barCounter;

      if(eventAge < 0 || eventAge > 20)
         return false;
     }

// Only closed candles are used for invalidation and retest.
   double retestPrice = iClose(_Symbol, tf, 1);
   double breakBuffer = MathMax(_Point * 2.0, atr * 0.02);

   for(int i = 3; i <= 60; i++)
     {
      double firstHigh = iHigh(_Symbol, tf, i + 1);
      double firstLow  = iLow(_Symbol, tf, i + 1);

      double thirdHigh = iHigh(_Symbol, tf, i - 1);
      double thirdLow  = iLow(_Symbol, tf, i - 1);

      datetime zoneTime = iTime(_Symbol, tf, i);

      if(zoneTime <= 0)
         continue;

      // Bullish IFVG:
      // 1. Original bearish FVG exists.
      // 2. A later closed candle breaks above its upper boundary.
      // 3. Price retests the former gap from above.
      if(bull)
        {
         bool bearishFVG =
            (thirdLow <= firstHigh);
         ...
         double zoneTop = firstLow;
         double zoneBottom = thirdHigh;

         if((zoneTop - zoneBottom) < atr * 0.15)
            continue;
         ...
         for(int b = i - 1; b >= 1; b--)
           {
            double closedPrice = iClose(_Symbol, tf, b);

            if(closedPrice > zoneTop + breakBuffer)
              {
               invalidated = true;
               break;
              }
         ...
         if(retestPrice >= zoneBottom &&
            retestPrice <= zoneTop)
           {
            outPrice = (zoneTop + zoneBottom) * 0.5;

            if(causal)
               outTag = g_smActiveCausalTag;
        ...
        {
         // Bearish IFVG:
         // 1. Original bullish FVG exists.
         // 2. A later closed candle breaks below its lower boundary.
         // 3. Price retests the former gap from below.
         bool bullishFVG =
            (thirdHigh >= firstLow);
         ...
         for(int b = i - 1; b >= 1; b--)
           {
            ...
            if(closedPrice < zoneBottom - breakBuffer)
              {
               invalidated = true;
               break;
              }
   ...
   if(causal)
     {
      ...
      if(!SM_GetRecentStructuralEvent(tfLay, 20, layerEvent))
         return false;
      if(layerEvent.direction != dir)
         return false;
      requiredTag = (g_smActiveCausalTag >= 0)
                    ? g_smActiveCausalTag : layerEvent.tag;
     }
...
//|                                                                  |
//+------------------------------------------------------------------+
bool SM_Detect_RejectionBlock(ENUM_TF_LAYER tfLay,
                              ENUM_TRADE_DIRECTION dir,
                              int &outTag,
                              double &outPrice)
  {
   outTag = -1;
   outPrice = 0.0;
   if(tfLay != LAYER_CTF || dir == DIR_NONE)
      return false;

   int idx = -1;
   if(!IsPriceAtRejectionBlock(dir == DIR_BULLISH, idx))
      return false;

   outPrice = (g_rejectionBlocks[idx].wickTop +
               g_rejectionBlocks[idx].wickBottom) * 0.5;
   return true;
  }
...
//|                                                                  |
//+------------------------------------------------------------------+
// NOTE: currently unreachable - dispatch routes SM_ELEM_SMT_DIVERGENCE
// to SM_Detect_SMTLayer() in ICT_MTFLayers.mqh (see M6). Kept in sync
// deliberately so the two definitions cannot drift again.
bool SM_Detect_SMTDivergence(ENUM_TRADE_DIRECTION dir)
  {
//--- FIX C8 (b): STRICT, matching SM_Detect_SMTLayer().
   if(dir == DIR_NONE)
      return false;
...
//--- FIX C4: fallback causal context must come from the stage's own
//    timeframe lane, never from whichever unrelated layer registered
//    last in the current CTF tick.
   SSMStructuralEvent causalEvent;
   int laneTag = SM_GetLatestStructuralEvent(tfLay, causalEvent)
                 ? causalEvent.tag : -1;
   g_smActiveCausalTag = (inst.birthEventTag >= 0)
                         ? inst.birthEventTag : laneTag;
   ...
   switch(elem)
     {
      ...
      case SM_ELEM_CHOCH_BREAK:
        {
         //--- FIX H16 ------------------------------------------------
         // The resolved event direction was computed and then silently
         // discarded, so a BEARISH ChoCh satisfied a BULLISH stage and
         // vice versa. This is the TRIGGER element of the DEFAULT
         // preset (SM_PRESET_CHOCH_RETRACE), so roughly half of all
         // spawned chains were structurally inverted from birth.
         // Mirrors the correct pattern already used by
         // SM_ELEM_JUDAS_SWING below.
         ENUM_TRADE_DIRECTION d = DIR_NONE;
         double p = 0.0;
         if(!SM_Detect_ChoChBreak(tfLay, d, p))
            return false;
         if(dir != DIR_NONE && d != dir)
            return false;
         SSMStructuralEvent chochEvent;
         outTag = SM_GetRecentStructuralEvent(tfLay, 5, chochEvent)
                  ? chochEvent.tag : -1;
         outPrice = p;
         return true;
        }
      ...
      case SM_ELEM_REJECTION_BLOCK:
         return SM_Detect_RejectionBlock(tfLay, dir, outTag, outPrice);
